...
@MainActor
public final class PerPRLazyCache<Value> {
    ...
    public init(fetch: @escaping @Sendable (String, String, Int) async -> Value?) { self.fetch = fetch }

    /// nil = not loaded yet; non-nil = loaded (may be empty).
    public func value(for pr: PullRequest) -> Value? { values[pr.key] }
    public func isLoading(_ pr: PullRequest) -> Bool { loading.contains(pr.key) }
    public func reset() { values.removeAll(); loading.removeAll() }
    ...
    public func load(_ pr: PullRequest, epoch: @escaping @autoclosure () -> Int,
              onReload: @escaping (String) -> Void) {
        guard values[pr.key] == nil, !loading.contains(pr.key) else { return }
        loading.insert(pr.key)
        let id = pr.key, (owner, repo) = pr.ownerRepo, number = pr.number, started = epoch()
        Task { [weak self] in
